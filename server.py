...
def get_ydl_options(cookie_file: str) -> dict:
    ...
    ydl_opts = {
        ...
        "noplaylist": True,
        "geo_bypass": True,
        "force_ipv4": True, # Added for cookie robustness
        "user_agent": (
...
async def download(video_url: str = Query(...),
    ...
    # --- HANDLE PURE MP3 AUDIO DOWNLOAD ---
    if ext.lower() == "mp3":
        ...
        try:
            ...
            if background_tasks:
                # FIX 1: Explicitly pass onerror=None to satisfy type checkers
                background_tasks.add_task(shutil.rmtree, tmpdir, ignore_errors=True, onerror=None)
    ...
    # FIX 3: Removed initial redundant 'video_path = None' declaration
    audio_path = None

    try:
        # --- Video download ---
        video_fmt = format_id or "bestvideo[ext!=mp4]+bestaudio[ext!=mp4]/best"
        ...
        video_path = files[0] # The 'video_path' variable is correctly used from here
        ...
        # cleanup later
        if background_tasks:
            # FIX 2: Explicitly pass onerror=None to satisfy type checkers
            background_tasks.add_task(shutil.rmtree, tmpdir, ignore_errors=True, onerror=None)
